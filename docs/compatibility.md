...
| Node fetch | v25.8.2 | automatic | `shellroute run US -- node -e "fetch('https://ipinfo.io/json').then(r=>r.json()).then(console.log)"` | Shellroute sets `NODE_USE_ENV_PROXY=1` automatically. Supported on Node 24.0+ (fetch) and 22.21+ (backport). Node 20 and older ignore it (fetch goes direct). Node 22 may emit an experimental warning. Opt out: `NODE_USE_ENV_PROXY=0`. |
| Node http/https | v25.8.2 | automatic | `shellroute run US -- node -e "const https=require('https'); https.get('https://ipinfo.io/json', r=>{let d=''; r.on('data',c=>d+=c); r.on('end',()=>console.log(d))})"` | Supported on Node 24.5+ and 22.21+ (backport) via default global agents. Custom agents can bypass proxy. |
